...
**팀명**: A.J.C 👉 [A]dvanced [J]oint documents [C]onverter 
...
| 이인재 | 백엔드 아키텍처 구성 및 데이터베이스 관리 | 구성된 파이프라인을 기반으로 전체 아키텍처에 코드를 나누어 배치하면서 서비스 흐름이 어떻게 흘러가는지 알게 되었고, 이를 통해 기능을 어디에 배치해야 하는지, 기능을 추가할 때 어떤 점을 고려해야 하는지 등의 유지보수하기 좋은 코드의 구조적 기준을 정립하게 되었습니다. |
